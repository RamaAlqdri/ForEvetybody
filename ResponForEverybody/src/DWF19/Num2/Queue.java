...
public class Queue {
    ...
    public void enqueue(String Nama, boolean Priority) {
    ...
    public void merge(Queue Priority, Queue NonPriority){
        ...
        for (int i=0;i<Priority.getDataSize();i++){
            enqueue(current.Nama, current.Priority);
            current=current.Next;
        }
    ...
    public void priority(){
        ...
        for (int i=0;i<getDataSize();i++){
            if (current.Priority==true){
                Priority.enqueue(current.Nama, current.Priority);
                dequeue();
            }else{
                NonPriority.enqueue(current.Nama, current.Priority);
                dequeue();
            }
            current=current.Next;
        }
